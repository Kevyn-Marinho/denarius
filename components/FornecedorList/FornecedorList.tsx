...
import { Container, Grid, Paper, ScrollArea, Table, ThemeIcon } from '@mantine/core';
import cx from 'clsx';
import classes from './FornecedorList.module.css';
...
const FornecedorList: React.FC<FornecedorListProps> = ({ fornecedores, onEdit, onDelete }) => {
  ...
  const rows = fornecedores.map((row) => (
    <Table.Tr key={row.id}>
      <Table.Td>{row.nome}</Table.Td>
      <Table.Td>{row.ramo}</Table.Td>
      <Table.Td>
        <ThemeIcon
              variant="light"
              size="lg"
